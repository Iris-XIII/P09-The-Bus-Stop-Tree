...
public class Bus implements Comparable<Bus> {
  ...
  public int compareTo(Bus other) {
    // compare arrive time (earlier < later)
    int time = this.arrivalTime.compareTo(other.arrivalTime);
    if (time != 0) {
      return time;
    }
    // compare routeAndTripName (alphabetical: a < b)
    int name = this.route.getRouteAndTripName().compareTo(other.route.getRouteAndTripName());
    if (name != 0) {
      return name;
    }
    // compare direction (outgoing < incoming)
    BusRoute.BusDirection outgoing = BusRoute.BusDirection.OUTGOING;
    BusRoute.BusDirection incoming = BusRoute.BusDirection.INCOMING;
    int dire = 0;
    if (this.route.getDirection().compareTo(outgoing) == 0
        && other.route.getDirection().compareTo(incoming) == 0) {
      dire = -1;
    } else if (this.route.getDirection().compareTo(incoming) == 0
        && other.route.getDirection().compareTo(outgoing) == 0) {
      dire = 1;
    }
    return dire;
  }
