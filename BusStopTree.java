...
public class BusStopTree {
  ...
  /**
   * private Helper: compare current node with the previous minimum and maximum parent node
   * 
   * @param node current node
   * @param min  minimum parent node
   * @param max  maximum parent node
   * @return true if it follow the rules of binary search tree
   */
  private static boolean isValidBST(Node<Bus> node, Bus minRight, Bus maxLeft) {
    // Base case: true for empty tree
    if (node == null) {
      return true;
    }
    // return false if node in right subtree is smaller than its minimum parent
    if ((minRight != null && node.getValue().compareTo(minRight) <= 0)) {
      return false;
    }
    // return false if the node in left subtree is greater than its maximum parent
    if ((maxLeft != null && node.getValue().compareTo(maxLeft) >= 0)) {
      return false;
    }
    // update maxLeft and minRight
    return isValidBST(node.getLeft(), minRight, node.getValue())
        && isValidBST(node.getRight(), node.getValue(), maxLeft);
  }
  ...
  public boolean addBusHelper(Node<Bus> node, Bus bus) {
    // compare node with bus
    int compare = node.getValue().compareTo(bus);

    // if node greater than bus, add or get left
    if (compare > 0) {
      if (node.getLeft() == null) {
        node.setLeft(new Node<Bus>(bus, null, null));
        return true;
      } else {
        return addBusHelper(node.getLeft(), bus);
      }
    }
    // if node less than bus, add or get right
    else if (compare < 0) {
      if (node.getRight() == null) {
        node.setRight(new Node<Bus>(bus, null, null));
        return true;
      } else {
        return addBusHelper(node.getRight(), bus);
      }
    }
    // Base case: if bus already in the tree, can't add
    else {
      return false;
    }
  }
  ...
  public boolean removeBus(Bus bus) {
    ...
    if (bus.compareTo(root.getValue()) == 0) {
      ...
      // The only non-success case where we newRoot can be null is when we are
      // removing the last
      // node in the tree. We handle that case in the if-case above. Here a null value
      // indicates
      // failure.
      return newRoot != null;
    }
  ...
  public boolean containsHelper(Node<Bus> node, Bus bus) {
    // Base case: subtree does not contain the bus
    if (node == null) {
      return false;
    }
    // compare the node and the bus
    int compare = node.getValue().compareTo(bus);
    // get left if node greater than bus
    if (compare > 0) {
      return containsHelper(node.getLeft(), bus);
    }
    // get right if node smaller than bus
    else if (compare < 0) {
  ...
  protected Node<Bus> getFirstNodeAfter(LocalTime time, Node<Bus> node) {
    // Base case: no bus found
    if (node == null) {
      return null;
    }

    // get the time of the node
    LocalTime currentTime = node.getValue().getArrivalTime();

    if (currentTime.compareTo(time) < 0) {
      // if current node arrive before desired time, get right
      return getFirstNodeAfter(time, node.getRight());
    } else {
      // if the current node arrives at or after or at desired time, get left and right to find the
      // soonest
      Node<Bus> left = getFirstNodeAfter(time, node.getLeft());
      Node<Bus> right = getFirstNodeAfter(time, node.getRight());

      // if left is null, node is the soonest
      if (left == null) {
        return node;
      }
      // if right is null, left is the soonest
      else if (right == null) {
        return left;
      }
      // if have both right and left, compare the them to get the soonest
      else {
        if (left.getValue().getArrivalTime().compareTo(right.getValue().getArrivalTime()) < 0) {
          return left;
        } else {
          return right;
        }
  ...
  public static Node<Bus> getFirstBusHelper(Node<Bus> node) {
    // if node has a left node, then process recursion
    if (node.getLeft() != null) {
      return getFirstBusHelper(node.getLeft());
    }
    // Base case: node is the first node in the tree
    return node;
  }
  ...
  public static Node<Bus> getLastBusHelper(Node<Bus> node) {
    // if node has a right node, then process recursion
    if (node.getRight() != null) {
      return getLastBusHelper(node.getRight());
    }
    // Base case: node is the last node in the tree
    return node;
  }
  ...
  private int heightHelper(Node<Bus> node) {
    ...
    // count the height from left side and right side
